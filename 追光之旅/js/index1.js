...
function line(){
	...
	$('.circle').unbind('click').bind("click",function(){
		...
		if(lastcircle.id==15-numline.length&&circle.id==lastcircle.id-1){
			if(numline.indexOf(lastcircle.id+"-"+circle.id)==-1){
				numline.push(lastcircle.id+"-"+circle.id);
				draw_line(lastcircle,circle);
			}
			if(numline.length==14){
...
function redraw() {
	...
	ctx.drawImage(img,config[frame+'.png'].frame.x,config[frame+'.png'].frame.y,config[frame+'.png'].frame.w,config[frame+'.png'].frame.h, width-config[frame+'.png'].spriteSourceSize.w*bi, config[frame+'.png'].spriteSourceSize.y*bi-65*bi, config[frame+'.png'].spriteSourceSize.w*bi,config[frame+'.png'].spriteSourceSize.h*bi);
}
