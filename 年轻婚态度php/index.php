...
if(isset($_GET['code']) && empty($_COOKIE['brn_marry_openid'])){
//if(1){
    ...
    setcookie('brn_marry_openid',$info['openid'],time()+31536000);
    setcookie('brn_marry_nickname',$info['nickname'],time()+31536000);
    setcookie('brn_marry_headimgurl',$info['headimgurl'],time()+31536000);
    $data = array(
        'openid'=>$info['openid'],
    ...
    //进行插入
    $member = $db->get('member',array('openid'=>$info['openid']),'id,nickname,headimgurl');
    if(!empty($member)){
...
{
    if(empty($_COOKIE['brn_marry_openid']))
    {
        //header('Location: index.php');
        $code=$wechatObj->get_code("http://weixin.fengme.cc/app/marry/index.php?".$_SERVER['QUERY_STRING'],TIMESTAMP);
        exit();
    }else
    {
        $data = array(
            'openid'=>$_COOKIE['brn_marry_openid'],
            'nickname' =>$_COOKIE['brn_marry_nickname'],
            'headimgurl' =>$_COOKIE['brn_marry_headimgurl'],
        );
        $member = $db->get('member',array('openid'=>$_COOKIE['brn_marry_openid']),'id');
